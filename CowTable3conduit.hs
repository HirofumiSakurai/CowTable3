...
getKineR = defaultLayout $ do
  setTitle "Rails compatible version"
  [whamlet|
<h1> Rails compatible version
<form action=@{KineJsonR} method="GET">
    name: <input name="search" type="text" value="">
    owner: <input name="search_owner" type="text" value="">
    <input type="submit" value="送信">
                 |]

getKineJsonR :: Handler TypedContent
getKineJsonR = do
  name' <- runInputGet $ iopt textField "search"
  owner' <- (runInputGet $ iopt textField "search_owner")
            ...
            $ E.from $ \(kine) -> do
                ...
                return $ (
                  ...
                  (kine ^. KineT41, kine ^. KineT42, kine ^. KineT43, kine ^. KineT44,
                   kine ^. KineT45, kine ^. KineT46, kine ^. KineT47, kine ^. KineT48,
                   kine ^. KineT49, kine ^. KineT50))
        toBuilder (E.Value earNum, E.Value name, E.Value birth,
