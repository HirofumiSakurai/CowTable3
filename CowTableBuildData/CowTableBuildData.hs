...
main = do
  ...
  disconnect conn
    where
        cowCols i = [toSql i, toSql i, toSql (name i), toSql (birthday i),
                     toSql "雌", toSql ((mod i 10) + 1)]
                      ++ map (\i -> toSql i) ([1..50] :: [Int])
        name i = "ハナ" ++ (printf "%05d" i)
